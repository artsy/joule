...
async function processHotjarRecordingMessages(client, event) {
  try {
    ...
    await client.chat.postMessage({
      channel: event.channel,
      thread_ts: event.ts,
      text: `<@${CLAUDE_USER_ID}>, please process this Hotjar recording as instructed. URL: ${url}`,
    });
  } catch (error) {
...
async function processMazeResponseMessages(client, event) {
  try {
    ...
    await client.chat.postMessage({
      channel: event.channel,
      thread_ts: event.ts,
      text: `<@${CLAUDE_USER_ID}>, please process this Maze response as instructed. URL: ${url}`,
    });
  } catch (error) {
